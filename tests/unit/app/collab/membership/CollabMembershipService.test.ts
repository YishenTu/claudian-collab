...
describe('CollabMembershipService', () => {
  ...
  it('retains the same frozen Cloud mutation after repeated unmarked stale rejections', async () => {
    await projects.saveMembership({
      authority: {
        authorityGeneration: 7,
        bindingVersion: 10,
  ...
  it('declines a Cloud responsibility against a pending Leave and replays the exact submitted request', async () => {
    ...
    await projects.saveMembership({
      ...
      authority: {
        ...membership.authority,
        gitRemoteUrl: 'https://other.example/v10/projects/project-alpha/repository.git',
