...
const DEVICE_SETTINGS_STORAGE_KEY = 'claudian-collab.deviceSettingsKey';
const LEGACY_DEVICE_SETTINGS_STORAGE_KEY = 'claudian.deviceSettingsKey';
...
function getDeviceSettingsSeed(): string {
  ...
  // Adopt this installation's old identity, never a synchronized vault identity.
  const candidate = storage.getItem(LEGACY_DEVICE_SETTINGS_STORAGE_KEY)?.trim()
    || createOpaqueDeviceSettingsSeed();
  try {
