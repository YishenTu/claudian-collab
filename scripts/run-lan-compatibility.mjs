...
const baselineRepository = 'https://github.com/YishenTu/claudian.git';
...
try {
  // The published LAN baseline belongs to Claudian's historical repository.
  // Resolve it in scratch space so this plugin needs neither that checkout nor its Git history.
  const baselineCheckout = path.join(workspace, 'baseline-repository');
  await mkdir(baselineCheckout);
  run('git', ['init', '--bare', baselineCheckout]);
  const git = (...args) => run('git', ['-C', baselineCheckout, ...args]);
  git('fetch', '--no-tags', '--depth=1', baselineRepository, baseline);
  const legacy = path.join(workspace, 'legacy');
  await mkdir(legacy);
  const archive = path.join(workspace, 'source.tar');
  await writeFile(archive, git(
    'archive', baseline, 'src', 'tsconfig.json', 'tests/helpers/installations.ts',
  ));
  run('tar', ['-xf', archive, '-C', legacy]);

  const lock = JSON.parse(git('show', `${baseline}:package-lock.json`).toString());
