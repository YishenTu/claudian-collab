...
- Use the Node version in `.node-version`. Run `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`, and `npm run test:build` for source changes; LAN binding changes also require `npm run test:lan-compatibility`.
